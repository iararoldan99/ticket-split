...
const SaldoGastos = ({ totalBudget, currentSpent }) => {
  return (
    <motion.div
    ...
    >
      <div className="flex flex-col items-center">
        ...
        <p className="text-4xl font-bold">${totalBudget.toFixed(2)}</p>
      </div>
      <div className="hidden md:block h-16 border-l border-black"></div>
      <div className="flex flex-col items-center">
        <h2 className="text-xl font-semibold">Gastos Registrados</h2>
        <p className="text-4xl font-bold text-red-600">-${currentSpent.toFixed(2)}</p>
      </div>
    </motion.div>
  );
};
